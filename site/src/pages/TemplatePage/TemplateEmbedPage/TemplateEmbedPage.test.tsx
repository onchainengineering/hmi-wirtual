...
test("Users can fill the parameters and copy the open in coder url", async () => {
	...
	expect(window.navigator.clipboard.writeText).toBeCalledWith(
		`[![Open in Coder](http://localhost/open-in-wirtual.svg)](http://localhost/templates/${MockTemplate.organization_name}/${MockTemplate.name}/workspace?mode=manual&param.first_parameter=firstParameterValue&param.second_parameter=123456)`,
	);
});
