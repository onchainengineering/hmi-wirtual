...
func (f *rotateFlags) attach(opts *serpent.OptionSet) {
	*opts = append(
		*opts,
		...
		serpent.Option{
			Flag:        "new-key",
			Env:         "WIRTUAL_EXTERNAL_TOKEN_ENCRYPTION_ENCRYPT_NEW_KEY",
			Description: "The new external token encryption key. Must be base64-encoded.",
			Value:       serpent.StringOf(&f.New),
		},
		serpent.Option{
			Flag:        "old-keys",
			Env:         "WIRTUAL_EXTERNAL_TOKEN_ENCRYPTION_ENCRYPT_OLD_KEYS",
...
func (f *decryptFlags) attach(opts *serpent.OptionSet) {
	*opts = append(
		*opts,
		...
		serpent.Option{
			Flag:        "keys",
			Env:         "WIRTUAL_EXTERNAL_TOKEN_ENCRYPTION_DECRYPT_KEYS",
...
func (f *deleteFlags) attach(opts *serpent.OptionSet) {
	*opts = append(
		*opts,
		serpent.Option{
			Flag:        "postgres-url",
			Env:         "WIRTUAL_EXTERNAL_TOKEN_ENCRYPTION_POSTGRES_URL",
			Description: "The connection URL for the Postgres database.",
			Value:       serpent.StringOf(&f.PostgresURL),
		},
		serpent.Option{
