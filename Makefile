...
FIND_EXCLUSIONS= \
	-not \( \( -path '*/.git/*' -o -path './build/*' -o -path './vendor/*' -o -path './.coderv2/*' -o -path '*/node_modules/*' -o -path '*/out/*' -o -path './wirtuald/apidoc/*' -o -path '*/.next/*' -o -path '*/.terraform/*' \) -prune \)
...
DB_GEN_FILES := \
	wirtuald/database/querier.go \
	wirtuald/database/unique_constraint.go \
	wirtuald/database/dbmem/dbmem.go \
	wirtuald/database/dbmetrics/dbmetrics.go \
	wirtuald/database/dbauthz/dbauthz.go \
	wirtuald/database/dbmock/dbmock.go
...
gen: \
	...
	vpn/vpn.pb.go \
	wirtuald/database/dump.sql \
	$(DB_GEN_FILES) \
	site/src/api/typesGenerated.ts \
	wirtuald/rbac/object_gen.go \
	...
	$(TAILNETTEST_MOCKS) \
	wirtuald/database/pubsub/psmock/psmock.go
.PHONY: gen
...
# applied.
wirtuald/database/dump.sql: wirtuald/database/gen/dump/main.go $(wildcard wirtuald/database/migrations/*.sql)
	go run ./wirtuald/database/gen/dump/main.go

# Generates Go code for querying the database.
# wirtuald/database/queries.sql.go
# wirtuald/database/models.go
wirtuald/database/querier.go: wirtuald/database/sqlc.yaml wirtuald/database/dump.sql $(wildcard wirtuald/database/queries/*.sql)
	./wirtuald/database/generate.sh

wirtuald/database/dbmock/dbmock.go: wirtuald/database/db.go wirtuald/database/querier.go
	go generate ./wirtuald/database/dbmock/

wirtuald/database/pubsub/psmock/psmock.go: wirtuald/database/pubsub/pubsub.go
	go generate ./wirtuald/database/pubsub/psmock
...
wirtuald/rbac/object_gen.go: scripts/typegen/rbacobject.gotmpl scripts/typegen/main.go wirtuald/rbac/object.go wirtuald/rbac/policy/policy.go
	go run scripts/typegen/main.go rbac object > wirtuald/rbac/object_gen.go

wirtualsdk/rbacresources_gen.go: scripts/typegen/wirtualsdk.gotmpl scripts/typegen/main.go wirtuald/rbac/object.go wirtuald/rbac/policy/policy.go
...
site/src/api/rbacresourcesGenerated.ts: scripts/typegen/wirtualsdk.gotmpl scripts/typegen/main.go wirtuald/rbac/object.go wirtuald/rbac/policy/policy.go
...
docs/admin/security/audit-logs.md: wirtuald/database/querier.go scripts/auditdocgen/main.go enterprise/audit/table.go wirtuald/rbac/object_gen.go
	go run scripts/auditdocgen/main.go
	./scripts/pnpm_install.sh
	pnpm exec prettier --write ./docs/admin/security/audit-logs.md

wirtuald/apidoc/swagger.json: $(shell find ./scripts/apidocgen $(FIND_EXCLUSIONS) -type f) $(wildcard wirtuald/*.go) $(wildcard enterprise/wirtuald/*.go) $(wildcard wirtualsdk/*.go) $(wildcard enterprise/wsproxy/wsproxysdk/*.go) $(DB_GEN_FILES) .swaggo docs/manifest.json wirtuald/rbac/object_gen.go
	./scripts/apidocgen/generate.sh
	./scripts/pnpm_install.sh
	pnpm exec prettier --write ./docs/reference/api ./docs/manifest.json ./wirtuald/apidoc/swagger.json
...
update-golden-files: \
	...
	wirtuald/.gen-golden \
	wirtuald/notifications/.gen-golden \
...
wirtuald/.gen-golden: $(wildcard wirtuald/testdata/*/*.golden) $(GO_SRC_FILES) $(wildcard wirtuald/*_test.go)
	go test ./wirtuald -run="Test.*Golden$$" -update
	touch "$@"

wirtuald/notifications/.gen-golden: $(wildcard wirtuald/notifications/testdata/*/*.golden) $(GO_SRC_FILES) $(wildcard wirtuald/notifications/*_test.go)
	go test ./wirtuald/notifications -run="Test.*Golden$$" -update
	touch "$@"
...
sqlc-push: sqlc-cloud-is-setup test-postgres-docker
	...
	sqlc push -f wirtuald/database/sqlc.yaml && echo "Passed sqlc push"
.PHONY: sqlc-push

sqlc-verify: sqlc-cloud-is-setup test-postgres-docker
	echo "--- sqlc verify"
	SQLC_DATABASE_URL="postgresql://postgres:postgres@localhost:5432/$(shell go run scripts/migrate-ci/main.go)" \
	sqlc verify -f wirtuald/database/sqlc.yaml && echo "Passed sqlc verify"
.PHONY: sqlc-verify

sqlc-vet: test-postgres-docker
	echo "--- sqlc vet"
	SQLC_DATABASE_URL="postgresql://postgres:postgres@localhost:5432/$(shell go run scripts/migrate-ci/main.go)" \
	sqlc vet -f wirtuald/database/sqlc.yaml && echo "Passed sqlc vet"
.PHONY: sqlc-vet
