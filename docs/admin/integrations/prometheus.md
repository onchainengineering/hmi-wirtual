...
either the environment variable `WIRTUAL_PROMETHEUS_ENABLE` or the flag
`--prometheus-enable`.
...
setting the environment variable `WIRTUAL_PROMETHEUS_ADDRESS` to `0.0.0.0:2112`.
The environment variable `WIRTUAL_PROMETHEUS_ENABLE` will be enabled
...
`WIRTUAL_PROMETHEUS_COLLECT_AGENT_STATS` before they can be retrieved from the
...
| `wirtuald_agentstats_session_count_ssh`                         | gauge     | The number of session established by SSH                                                                                         | `agent_name` `username` `workspace_name`                                             |
...
| `wirtuald_oauth2_external_requests_rate_limit_next_reset_unix`  | gauge     | Unix timestamp of the next interval                                                                                              | `name` `resource`                                                                    |
