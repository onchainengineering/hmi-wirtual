...
```env
WIRTUAL_OIDC_ISSUER_URL="https://issuer.corp.com"
WIRTUAL_OIDC_EMAIL_DOMAIN="your-domain-1,your-domain-2"
WIRTUAL_OIDC_CLIENT_ID="533...des"
WIRTUAL_OIDC_CLIENT_SECRET="G0CSP...7qSM"
```
...
To troubleshoot claims, set `WIRTUAL_VERBOSE=true` and follow the logs while
...
> option `WIRTUAL_OIDC_IGNORE_USERINFO=true`.
...
`WIRTUAL_OIDC_EMAIL_FIELD` to the desired claim.
...
```env
WIRTUAL_OIDC_IGNORE_EMAIL_VERIFIED=true
```
...
`WIRTUAL_OIDC_USERNAME_FIELD` to the desired claim.
...
```env
WIRTUAL_OIDC_SIGN_IN_TEXT="Sign in with Gitea"
WIRTUAL_OIDC_ICON_URL=https://gitea.io/images/gitea.png
```
...
```env
WIRTUAL_DISABLE_PASSWORD_AUTH=true
```
...
```env
WIRTUAL_SCIM_AUTH_HEADER="your-api-key"
```
...
```env
WIRTUAL_TLS_CLIENT_CERT_FILE=/path/to/cert.pem
WIRTUAL_TLS_CLIENT_KEY_FILE=/path/to/key.pem
```
