	...
	cmd := &serpent.Command{
		...
		Handler: func(inv *serpent.Invocation) error {
			...
			if username == "" {
				username, err = cliui.Prompt(inv, cliui.PromptOptions{
					...
					Validate: func(username string) error {
						err = wirtualsdk.NameValid(username)
						if err != nil {
			...
			if name == "" && shouldPromptName {
				...
				name = wirtualsdk.NormalizeRealUsername(rawName)
			...
			if disableLogin {
				...
				userLoginType = wirtualsdk.LoginType(loginType)
			}

			if password == "" && userLoginType == wirtualsdk.LoginTypePassword {
			...
			_, err = client.CreateUserWithOrgs(inv.Context(), wirtualsdk.CreateUserRequestWithOrgs{
				Email:           email,
				Username:        username,
				Name:            name,
				Password:        password,
			...
			switch wirtualsdk.LoginType(strings.ToLower(string(userLoginType))) {
	...
	cmd.Options = serpent.OptionSet{
		...
		},
		{
			...
			Description: fmt.Sprintf("Optionally specify the login type for the user. Valid values are: %s. "+
				...
				strings.Join([]string{
					string(wirtualsdk.LoginTypePassword), string(wirtualsdk.LoginTypeNone), string(wirtualsdk.LoginTypeGithub), string(wirtualsdk.LoginTypeOIDC),
				}, ", ",
				)),
